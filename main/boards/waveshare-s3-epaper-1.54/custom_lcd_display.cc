...
void CustomLcdDisplay::example_lvgl_flush_cb(lv_display_t * disp, const lv_area_t * area, uint8_t * color_p)
{
    assert(disp != NULL);
...
CustomLcdDisplay::CustomLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    lv_display_set_flush_cb(display_, example_lvgl_flush_cb);
...
void CustomLcdDisplay::Power_label_OFF() {
    lv_label_set_text(power_label_, "OFF");
    lv_obj_invalidate(power_label_);
